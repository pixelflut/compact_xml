...
module PxBuilder
  ...
  class XmlMarkup < XmlBase
    ...
    # Returns symbol<>String or CamelCased String
    def _element(sym)
      if @camelcase
        sym.to_s.gsub(/([_-]{1})([A-Za-z0-9]{1})/) { $2.upcase }
      else
        sym
      end
    end
    ...
    def _insert_attributes(attrs, order=[])
      ...
      order.each do |k|
        v = attrs[k]
        @target << %{ #{_element(k)}="#{_attr_value(v)}"} if v # " WART
      end
      attrs.each do |k, v|
        @target << %{ #{_element(k)}="#{_attr_value(v)}"} unless order.member?(k) # " WART
      end
    end
